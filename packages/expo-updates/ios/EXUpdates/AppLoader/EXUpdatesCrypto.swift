...
public class EXUpdatesCrypto {
  private static let PublicKeyUrl = "https://exp.host/--/manifest-public-key"
  ...
  public static func verifySignature(
    ...
    fileDownloader.downloadData(
      fromURL: URL(string: PublicKeyUrl)!,
      extraHeaders: [:]
    ) { publicKeyData, response in
  ...
  private static func verify(
    ...
    DispatchQueue.main.async {
      let publicKey = keyRefFromPEMData(publicKeyData)
      let signatureData = NSData(base64Encoded: signature)! as Data
      let signedData = signedString.data(using: .utf8)!
      var isValid = false
      if let publicKey = publicKey {
        isValid = verifyRSASHA256SignedData(signedData, signatureData: signatureData, publicKey: publicKey)
      }
      callback(isValid)
    }
  }
