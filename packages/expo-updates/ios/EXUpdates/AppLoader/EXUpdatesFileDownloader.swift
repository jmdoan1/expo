...
public final class EXUpdatesFileDownloader: NSObject, URLSessionDataDelegate {
  ...
  public func parseManifestResponse(
    ...
    if contentType.lowercased().hasPrefix("multipart/") {
      let contentTypeParameters = EXUpdatesParameterParser().parseParameterString(contentType, withDelimiter: ";".utf16.first!) as! [String: Any]
      guard let boundaryParameterValue: String = contentTypeParameters.optionalValue(forKey: "boundary") else {
  ...
  private func parseMultipartManifestResponse(
    ...
    let completed = reader.readAllParts { headers, content, _ in
      if let contentDisposition = (headers as! [String: Any]).stringValueForCaseInsensitiveKey("content-disposition") {
        let contentDispositionParameters = EXUpdatesParameterParser().parseParameterString(
          contentDisposition,
          withDelimiter: ";".utf16.first!
        ) as! [String: Any]
        if let contentDispositionNameFieldValue: String = contentDispositionParameters.optionalValue(forKey: "name") {
  ...
  func parseManifestBodyData(
    ...
    do {
      manifest = try JSONSerialization.jsonObject(with: manifestString.data(using: .utf8)!) as? [String: Any]
    } catch {
